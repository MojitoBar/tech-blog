...
:::tip[핵심 요약]
SQL **윈도우 함수(Window Function)** 는 복잡한 데이터 분석을 위한 강력한 도구입니다. 

결과 집합의 행 수를 변경하지 않고, 각 행에 대해 특정 범위(윈도우)의 값들을 기준으로 계산한 결과를 보여주어, 순위, 누적 합계, 이동 평균, 행 간 비교 등 다양한 분석을 간결하게 수행할 수 있게 합니다.

이 글에서는 윈도우 함수의 기본 개념, 주요 함수 유형과 활용법, 그리고 성능을 고려한 사용 전략까지 상세히 안내합니다.
:::
...
과거에는 이런 분석을 위해 복잡한 서브쿼리나 여러 번의 조인이 필요했지만, SQL 윈도우 함수를 사용하면 훨씬 간결하고 효율적으로 해결할 수 있습니다.

이 글에서는 SQL 윈도우 함수의 핵심 개념부터 실제 활용법, 그리고 성능까지 고려한 사용 전략까지 깊이 있게 살펴보겠습니다. 윈도우 함수를 제대로 이해하고 활용한다면, 데이터 분석의 새로운 가능성을 발견하고 업무 효율성을 크게 높일 수 있을 것입니다.
...
덕분에 개별 행의 상세 정보와 함께 집계된 분석 값을 한눈에 볼 수 있다는 큰 장점이 있습니다.
...
윈도우 함수는 그 기능에 따라 여러 유형으로 나눌 수 있습니다. 여기서는 가장 자주 사용되는 핵심 함수들을 실제 활용 예시와 함께 살펴보겠습니다.

### 순위 매기기: `RANK()`, `DENSE_RANK()`, `ROW_NUMBER()`

그룹 내에서 각 행의 순위를 매길 때 사용합니다. `ORDER BY` 절이 필수적입니다.
...
일반 집계 함수처럼 합계, 평균, 개수 등을 계산하지만, `GROUP BY` 절 없이 사용되며 각 행에 대한 결과를 반환합니다. `PARTITION BY`로 정의된 윈도우 내에서 집계가 이루어집니다.
...
현재 행을 기준으로 특정 파티션 내에서 정렬된 순서에 따라 이전 행 또는 다음 행의 값을 가져올 때 사용합니다. 시계열 데이터 분석에서 변화량을 계산하거나 추세를 파악하는 데 매우 유용합니다. `ORDER BY` 절이 필수적입니다.
...
윈도우 함수는 매우 편리하지만, 내부적으로 정렬(Sort)이나 복잡한 연산을 수행할 수 있어 대용량 데이터 처리 시 성능에 영향을 줄 수 있습니다. 따라서 효율적인 사용을 위해 몇 가지 사항을 고려해야 합니다.

*   **필요한 데이터만 사용하기**
    *   윈도우 함수를 적용하기 전에 `WHERE` 절을 사용해 분석 대상을 최대한 줄이는 것이 좋습니다. 데이터가 적을수록 윈도우 계산도 빨라집니다.
    *   서브쿼리나 CTE(Common Table Expression)를 활용하여 윈도우 함수가 처리할 레코드 수를 미리 줄이는 것도 효과적입니다.
*   **적절한 인덱스 활용하기**
    *   `PARTITION BY` 절이나 `ORDER BY` 절에 사용되는 컬럼에 인덱스가 있다면 성능 향상에 도움이 될 수 있습니다. 데이터베이스 옵티마이저가 인덱스를 활용하여 파티셔닝이나 정렬 작업을 더 빠르게 수행할 수 있기 때문입니다.
*   **불필요한 정렬 최소화하기**
    *   모든 윈도우 함수가 `ORDER BY`를 필요로 하지는 않습니다. (예: `SUM() OVER (PARTITION BY ...)`). `ORDER BY`가 특정 함수의 기능(예: 순위 함수, `LAG`/`LEAD`)이나 `WINDOWING_CLAUSE`의 논리적 범위 지정에 꼭 필요한 경우에만 사용하고, 불필요한 정렬은 피해야 합니다.
*   **윈도우 프레임 현명하게 사용하기**
    *   `WINDOWING_CLAUSE`에서 `UNBOUNDED PRECEDING`이나 `UNBOUNDED FOLLOWING`과 같이 매우 넓은 범위를 지정하면 계산량이 많아질 수 있습니다. 특히 `RANGE` 기반 프레임에서 정렬 기준 값에 중복이 많을 경우 예기치 않은 성능 저하가 발생할 수 있습니다.
    <br/>가능한 경우 필요한 만큼의 범위로 제한하거나(`ROWS BETWEEN 2 PRECEDING AND CURRENT ROW` 등), 데이터 특성을 고려하여 `ROWS` 또는 `RANGE`를 신중하게 선택해야 합니다.

테이블 스캔 횟수가 적다는 이유만으로 윈도우 함수가 항상 다른 방법(예: 여러 번의 조인이나 셀프 조인, 스칼라 서브쿼리)보다 성능이 우수하다고 단정할 수는 없습니다. 

실제 실행 계획을 확인하고, 데이터의 특성, 양, 그리고 DBMS의 최적화 능력에 따라 최적의 방식을 선택하는 것이 중요합니다.

## 더 빠른 SQL을 위한 일반적인 쿼리 최적화 팁

윈도우 함수 사용과 더불어, 다음과 같은 일반적인 SQL 쿼리 최적화 팁을 함께 적용하면 전반적인 데이터 처리 성능을 높이는 데 도움이 됩니다.

*   **`WHERE` 절 컬럼 가공 피하기 (SARGable 쿼리 작성)**:
    `WHERE` 절에서 인덱스가 걸린 컬럼에 함수를 사용하거나 연산을 수행하면(예: `WHERE SUBSTRING(indexed_column, 1, 3) = 'ABC'` 또는 `WHERE indexed_column + 1 = 100`), 데이터베이스가 인덱스를 효과적으로 사용하지 못하고 전체 테이블 스캔으로 이어질 가능성이 큽니다. 대신 조건을 우변으로 옮기거나, 가공되지 않은 원본 컬럼을 기준으로 비교하도록 쿼리를 수정하세요. (예: `WHERE indexed_column LIKE 'ABC%'` 또는 `WHERE indexed_column = 99`)
*   **`OR` 조건 신중히 사용하기 (때로는 `UNION ALL` 고려)**:
    복잡한 `OR` 조건은 인덱스 활용을 어렵게 만들 수 있습니다. 각 조건이 독립적이고 서로 다른 인덱스를 활용할 수 있다면, `UNION ALL`로 쿼리를 분리하여 실행하는 것이 더 효율적일 수 있습니다. (단, `UNION ALL`은 중복을 제거하지 않으므로 주의해야 하며, `UNION`은 중복 제거를 위한 추가 비용이 발생합니다.)
*   **필요한 행과 열만 선택하기**:
    `SELECT *` 대신 실제로 필요한 컬럼만 명시하고, `WHERE` 절을 사용하여 필요한 행만 가져오도록 필터링하는 것은 가장 기본적인 성능 최적화 방법입니다. 불필요한 데이터 전송량과 메모리 사용량을 줄일 수 있습니다.
*   **`LIKE` 연산 시 와일드카드(`%`) 위치 주의하기**:
    `LIKE` 연산자 사용 시, 와일드카드(`%`)를 검색 문자열 앞에 사용하면(예: `LIKE '%text'`) 인덱스를 활용하기 매우 어렵습니다. 가능한 경우 문자열 뒤에 와일드카드를 사용하는 것이(예: `LIKE 'text%'`) 인덱스 스캔(Index Range Scan)을 유도하여 검색 성능에 유리합니다.
*   **반복 계산 줄이기 (계산된 값 미리 저장 또는 CTE 활용)**:
    매우 복잡하고 자주 사용되는 계산 결과나 표현식은 실시간으로 매번 계산하기보다, 뷰(View), 구체화된 뷰(Materialized View), 임시 테이블 또는 CTE 내에서 한 번 계산하여 재활용하는 것이 성능에 유리할 수 있습니다.

## 🏁 결론

SQL 윈도우 함수는 복잡해 보이는 데이터 분석 요구사항을 간결하고 우아하게 해결할 수 있는 강력한 도구입니다. 단순히 순위를 매기고 집계를 하는 것을 넘어, 데이터 행 간의 관계를 파악하고 시계열 분석, 비율 계산 등 다양한 고급 분석을 SQL만으로 수행할 수 있게 해줍니다.

오늘 살펴본 것처럼 윈도우 함수의 기본 개념과 문법을 정확히 이해하고, 다양한 함수 유형을 적재적소에 활용하며 성능 측면까지 고려한다면, 데이터 분석의 효율성과 깊이를 한층 끌어올릴 수 있을 것입니다. 윈도우 함수의 세계는 넓고 깊으니, 꾸준히 탐구하고 여러 상황에 적용해보면서 그 진정한 힘을 경험해보시길 바랍니다.
